...
int open_dads_trace_file(pstr_trace pt){
	char path[256];
...
int close_dads_trace_file(pstr_trace pt){
	fclose(pt->fTrace);
	pt->fTrace=NULL;
	return ERR_SUCCESS_AC;
}

int get_dads_record(pstr_trace pt, pstr_user_request pur){
	char hostname[50];
	char rw;
	long response;
	//printf("get_dads_record begin\n");
	pthread_mutex_lock(&pt->trace_mutex);
	//printf("get_dads_record locked\n");
	...
	pthread_mutex_unlock(&pt->trace_mutex);
	//printf("get_dads_record end\n");
	return ERR_SUCCESS_AC;
}

void print_dads_user_request(pstr_user_request pur){
	printf("ts:%lf\t",pur->ts);
...
static struct trace dads_trace =
{
	.name							= "cam-dads",
	.path							= "/trace/cam-dads/dads",
	.plus							= "",
	.cnt								=	0,
	.recovery_start_ts		=	0,
	.warmup_lines				=	200000,
	.speedup_rate				=	2.0,
	.fTrace							=	NULL,
	.open_trace_file			= open_dads_trace_file,
	.close_trace_file			=	close_dads_trace_file,
	.get_record					= get_dads_record,
	.print_record				= print_dads_user_request,
};

//global entrance
int connect_dads_trace(void* pac){
	((pstr_acache)pac)->tpers=(void*)&dads_trace;
	dads_trace.pacache=pac;
	return ERR_SUCCESS_AC;
}
