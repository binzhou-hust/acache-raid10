...
int open_lmtbe_trace_file(pstr_trace pt){
	char path[256];
...
int close_lmtbe_trace_file(pstr_trace pt){
	fclose(pt->fTrace);
	pt->fTrace=NULL;
	return ERR_SUCCESS_AC;
}

int get_lmtbe_record(pstr_trace pt, pstr_user_request pur){
	char hostname[50];
	char rw;
	long response;
	//printf("get_lmtbe_record begin\n");
	pthread_mutex_lock(&pt->trace_mutex);
	//printf("get_lmtbe_record locked\n");
...
void print_lmtbe_user_request(pstr_user_request pur){
	printf("ts:%lf\t",pur->ts);
...
static struct trace lmtbe_trace =
{
	.name							= "ms-lmtbe",
	.path							= "/trace/ms-lmtbe/lmtbe",
	.plus								= "",
	.cnt								=	0,
	.recovery_start_ts		=	0,
	.warmup_lines				=	200000,
	.speedup_rate				=	2.0,
	.fTrace							=	NULL,
	.open_trace_file			= open_lmtbe_trace_file,
	.close_trace_file			=	close_lmtbe_trace_file,
	.get_record					= get_lmtbe_record,
	.print_record				= print_lmtbe_user_request,
};

//global entrance
int connect_lmtbe_trace(void* pac){
	((pstr_acache)pac)->tpers=(void*)&lmtbe_trace;
	lmtbe_trace.pacache=pac;
	return ERR_SUCCESS_AC;
}
